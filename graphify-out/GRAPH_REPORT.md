...
## Summary
- 427 nodes · 798 edges · 33 communities (23 shown, 10 thin omitted)
...
8. `validate_safe_path()` - 17 edges
9. `developer_node()` - 16 edges
...
### Community 1 - "FactoryState"
Cohesion: 0.09
Nodes (34): auditor_should_continue_router(), Envía las llamadas del Auditor al ejecutor antes de aplicar la política., build_tool_executor_node(), _call_data(), developer_node(), LocalToolNode, _message_content(), Any (+26 more)
...
Cohesion: 0.08
Nodes (24): auditor_node(), _calculate_completion_percentage(), _extract_audit_decision(), Any, Nodo de auditoría segura para validar el trabajo del desarrollador., Extrae una decisión sin aprobar por accidente una respuesta ambigua., Lee el checklist conocido sin salir del workspace seguro., Audita el estado actual y devuelve aprobación, informe y avance. (+16 more)

### Community 4 - "__init__.py"
Cohesion: 0.11
Nodes (24): ArgumentParser, build_parser(), main(), Punto de entrada para ejecutar la fábrica sobre un repositorio local., Componentes compartidos de la fábrica de software., create_model(), _environment(), _load_chat_openai() (+16 more)
...
- **Why does `create_initial_state()` connect `auditor.py` to `FactoryState`, `FactoryStateContractTests`, `__init__.py`, `.test_auditor_ejecuta_herramienta_y_recibe_su_resultado`, `test_workflow_cycle.py`?**
  _High betweenness centrality (0.048) - this node is a cross-community bridge._
- **Why does `FactoryState` connect `FactoryState` to `FactoryStateContractTests`, `auditor.py`, `__init__.py`, `architect_node`?**
  _High betweenness centrality (0.047) - this node is a cross-community bridge._
- **Why does `github_secure_push_tool()` connect `github_secure_push_tool` to `auditor.py`, `__init__.py`, `ensure_commit_approved`?**
...
- **Are the 2 inferred relationships involving `build_autonomous_factory()` (e.g. with `should_continue_router()` and `consolidate_developer_evidence()`) actually correct?**
  _`build_autonomous_factory()` has 2 INFERRED edges - model-reasoned connections that need verification._
