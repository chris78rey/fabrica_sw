...
try:
    ...
    # Streamlit runs this file directly, without package context.
    # Add `src` so the absolute import can resolve in that mode.
...
def main() -> None:
    try:
        ...
        raise SystemExit(
            "Falta Streamlit. Instala la interfaz con: "
    ...
    if "requirements_file" not in st.session_state:
        st.session_state["requirements_file"] = ""
    ...
    with st.sidebar:
        ...
        repository = st.text_input(
            "Repositorio", key="repository", placeholder=r"G:\codex_projects\mi_proyecto"
        )

        st.button(
            "Seleccionar archivo",
            on_click=_select_requirements_file,
            use_container_width=True,
        )
        requirements_file = st.text_input(
            "Archivo de requerimientos",
            key="requirements_file",
            placeholder=r"G:\codex_projects\mi_proyecto\inst.md",
        )
    ...
    can_run = bool(
        ...
        and requirements_path
        and _path_is_readable_file(requirements_file)
    )
    if st.button("Iniciar fábrica", type="primary", disabled=not can_run, use_container_width=True):
        try:
            ...
            with st.status("Ejecutando Arquitecto → Desarrollador → Auditor...", expanded=True) as status:
                result = run_factory(
                    FactoryRunRequest(
                ...
                status.update(label=f"Ejecución terminada: {result.report['status']}", state="complete")
